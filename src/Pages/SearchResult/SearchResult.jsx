...
const SearchResult = (props) => {
	...
	const loadData = () => {
		...
		if (filter.transit !== '') {
			...
			const result = dataApi.filter(
				(ticket) =>
					Number(ticket.times.berangkat.split(':')[0]) > Number(start[0]) &&
			...
			const result = dataApi.filter(
				(ticket) =>
					Number(ticket.times.tiba.split(':')[0]) > Number(start[1]) &&
			...
			const result = dataApi.filter(
				(ticket) =>
					String(ticket.Maskapai.nameMaskapai).toLocaleLowerCase() ===
